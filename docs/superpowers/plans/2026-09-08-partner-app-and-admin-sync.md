# BlooRush Partner App & Real-Time Admin Sync — Implementation Plan
...
**Spec:** [`docs/PARTNER_APP_ARCHITECTURE.md`](file:///C:/Users/hp/Documents/Bloorush_App/docs/PARTNER_APP_ARCHITECTURE.md)

---

## Global Constraints

- **Dual-App Separation**: Customer App (`com.bloorush.customer`) and Partner App (`com.bloorush.partner`) must have distinct package IDs so both can be installed on the same phone.
- **Admin Panel Web-Only**: `admin.html` and `admin.js` must be excluded from mobile app bundles for security and compactness.
- **Java 21**: GitHub Actions Android CI must use Java 21 (`temurin` 21).
- **Non-Interactive CI**: Always use `npx --yes cap ...` in automated build scripts.
- **Real-Time Data Source**: Both mobile apps and the admin panel must share the exact same Firebase project (`mvp-bloorush`) and Firestore collections (`jobs`, `slots`, `partners`, `attendance`).
...
- **Create**:
  - `scripts/build-partner.js` — Bundles partner web assets (`partner.html`, `partner.js`, `data.js`, `styles.css`, `mobile-bridge.js`) into `www-partner/` while excluding admin files.
  ...
  - `docs/superpowers/plans/2026-09-08-partner-app-and-admin-sync.md` — This implementation plan.
- **Modify**:
  - `.gitignore` — Ensure `*.apk` and temporary build directories are ignored.
  - `partner-test/partner.html` — Update asset paths for native bundling compatibility.
...
**Interfaces:**
- Keeps local repo clean; ignores built APK binaries (`*.apk`) and `www-partner/`.
...
- [ ] **Step 2: Enhance `partner-test/partner.js` with native haptics**
  Update `vibrate()` to call `window.BlooRushNative.hapticImpact()` if running natively.
...
### Task 3: Partner Capacitor Configuration & Android Project Setup
...
- [ ] **Step 2: Generate native Android partner project**
